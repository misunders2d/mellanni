...
def plot_temp_old(plot_cols,f,pivot):
    sns.relplot(x = plot_cols[0], y = plot_cols[1], data = f,
                hue = plot_cols[2], size = plot_cols[3], style = plot_cols[4], col = plot_cols[5])
    plt.suptitle('All sales')
    # plt.show()
    plt.savefig('plot.png')
    plt.close()
    sns.relplot(x = plot_cols[0], y = plot_cols[1], data = pivot,
                hue = plot_cols[2], size = plot_cols[3], style = plot_cols[4], col = plot_cols[5])
...
def read_file():
    ...
    event,sheet = sg.Window('Select data sheet',
                      layout = [
                          [sg.DropDown(sheets, default_value=sheets[0],
                                       size = btn, change_submits = True)]],
                      size = (300,50)
                      ).read(close = True)
...
def create_data_cols(f):
    ...
    event,d_cols = sg.Window(
        'Select data columns',
        layout = [
            [sg.Text('Sales monthly', s = txt), sg.DropDown(cols, key = 'SM',default_value = 'Sales')],
            [sg.Text('Sales daily', s = txt), sg.DropDown(cols, key = 'SD'),sg.Checkbox('generate', key = 'GEN', default = True)],
            [sg.Text('BSR', s = txt), sg.DropDown(cols, key = 'BSR',default_value = 'BSR')],
            [sg.Text('Price', s = txt), sg.DropDown(cols, key = 'PRICE', default_value = 'Price $'),
    ...
    if event == 'BACK':
        ...
        f[d_cols] = f[d_cols].applymap(lambda x: str(x).replace('"','').replace(' ','').replace('\xa0','').replace(',',''))
...
def create_index_cols(cols):
    event, i_cols = sg.Window('Select index (aggregation) columns',
                      layout = [
...
def pivot_create(f,i_cols,d_cols):
    ...
    price_bins = [0,2,3,5,10,15,20,25,30,35,40,45,50,60,70,80,90,100,120,150,200,250,300,350,400,500]
    ...
    variation_counts = f.pivot_table(
        ...
        aggfunc = {
            ...
            'Sales': lambda x: x.sort_values(ascending = False).cumsum().le(x.sum()*.8).sum()+1
    ...
    variation_counts.rename(columns = {'ASIN':'Variation count','Sales':'Contributing variations'}, inplace = True)
    
    f = pd.merge(f, variation_counts, how = 'left', on = ['Brand','Total Sales'])
    
    f['# of variations'] = pd.cut(f['Variation count'],variation_bins)
    f['% of contributing variations'] = pd.cut(round(f['Contributing variations'] / f['Variation count']*100,0),contribution_bins)
    result = f.copy()
    
    result = result.sort_values(['Total Sales','Brand','Sales daily'], ascending = [False,True,False])
    result['Listing share'] = round(result['Total Sales'] / result['Sales'].sum()*100,1)
    
    pivot = result[d_cols + i_cols].copy()
    pivot[i_cols] = pivot[i_cols].applymap(lambda x: str(x).replace('"','').replace("'","").strip())
    ...
    pivot['Sales share'] = round((pivot['Sales sum']/pivot['Sales sum'].sum())*100,1)
    
    i_cols +=['# of variations','% of contributing variations']
    pivot2 = result[d_cols + i_cols].copy()
    pivot2[i_cols] = pivot2[i_cols].applymap(lambda x: str(x).replace('"','').replace("'","").strip())
    ...
    pivot2['Sales share'] = round((pivot2['Sales sum']/pivot2['Sales sum'].sum())*100,1)
...
def plot_preview(f,i_cols, d_cols, pivot):
    ...
    # plot_size = (800,400)
    btn_col = [
        [sg.Text('x axis', size = txt2), sg.DropDown(i_cols, default_value = d_cols[-1], change_submits=True, key = 'X')],
        [sg.Text('y axis', size = txt2), sg.DropDown(i_cols, default_value = d_cols[0], change_submits=True, key = 'Y')],
        [sg.Text('Color', size = txt2), sg.DropDown(i_cols, default_value = None, change_submits=True, key = 'COLOR')],
        [sg.Text('Size', size = txt2), sg.DropDown(i_cols, default_value = None, change_submits=True, key = 'SIZE')],
        [sg.Text('Style', size = txt2), sg.DropDown(i_cols, default_value = None, change_submits=True, key = 'STYLE')],
        [sg.Text('Columns', size = txt2), sg.DropDown(i_cols, default_value = None, change_submits=True, key = 'COLS')]
        ]
    plot_image = sg.Image(source = os.path.join(prefix,r'70 Data & Technology\70.03 Scripts\mellanni.png'))#, size = plot_size)
    plot2_image = sg.Image(source = os.path.join(prefix,r'70 Data & Technology\70.03 Scripts\mellanni.png'))#, size = plot_size)
    img_col = [[plot_image],
               [plot2_image]]
    
    layout = [
        [sg.Column(btn_col),sg.Column(img_col)],#, scrollable = True, size = (plot_size[0],plot_size[1]*2))],
        [sg.Button('OK', change_submits=True)]
        ]
    window = sg.Window('Select the plot layout', layout, size = (1000,700), resizable=True)
    while True:
        event, values = window.read()
    
        if event == sg.WIN_CLOSED or event == 'Cancel': # if user closes window or clicks cancel
            break
        elif any([event == 'COLOR',event == 'SIZE',event == 'STYLE',event == 'COLS',event == 'X',event == 'Y']):
            x,y,hue, size, style, col = values['X'],values['Y'],values['COLOR'],values['SIZE'],values['STYLE'],values['COLS']
...
def main_func():
    ...
    while 'plot.png' in os.listdir(os.getcwd()) or 'plot2.png' in os.listdir(os.getcwd()):
        try:
            os.remove('plot.png')
            os.remove('plot2.png')
        except:
